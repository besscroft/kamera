...
<template>
  <dialog id="kamera_modal" class="modal w-full h-full">
    <div class="modal-box rounded w-full h-full max-w-full max-h-full base-100 scrollbar-hide">
      ...
      <div :class="smAndLarger ? 'grid grid-cols-1 gap-2 md:grid-cols-3 lg:gap-4' : 'h-full flex flex-col pt-6 space-y-2'">
        <div :class="smAndLarger ? 'md:col-span-2 max-h-full' : ''">
          <img class="max-h-full" :src="dataList[imgId-1]?.url" />
        </div>
        <div class="flex flex-col space-y-4 mt-8">
          <div class="flex flex-col card w-full h-16 p-2 bg-base-100 shadow-md shadow-slate-100 hover:bg-slate-50 justify-center items-center">
            <span>EXIF</span>
            <span>略...</span>
          </div>
          <div class="flex flex-col card w-full h-16 p-2 bg-base-100 shadow-md shadow-slate-100 hover:bg-slate-50 justify-center items-center">
